...
const sortStarWarsCharacters = (starWarsArr) => {
  return starWarsArr.sort((heightOne, heightTwo) => { return heightTwo.height - heightOne.height; });
};
...
const listFoods = (recipe) => {
  let result = [];
  let Array2 = recipe.ingredients;
  for (var i = 0; i < Array2.length; i++) {
    let index = Array2[i].indexOf(' ', 3);
    let Array1 = Array2[i].slice(index + 1, Array2[i].length);
    result.push(Array1);
  }
  return result;
};
