...
function transformToLis(obj) {
  return Object.entries(obj).map((element) => `<li>${element.join(': ')}</li>`);
...
const divisibleByFiveTwoToThePower = (input) => {
  let newArray = [];
  newArray = input.map(
    Element => Element.filter(e => (typeof e === 'number' && e % 5 === 0)));
  return newArray.map(result1 => result1.map(result2 => Math.pow(2, result2)))
...
let findMaleAndFemale = (data) => {
  let newData = [];
  newData = data.filter(Element => (Element.gender === 'female') || (Element.gender === 'male'));
  return newData.map(Element2 => Element2.name).join(' and ');
};
...
let findShortest = (data) => {
  let short = data.reduce((value1, value2) => (Number(value1.height) < Number(value2.height)) ? value1 : value2);
  return short.name;
};
