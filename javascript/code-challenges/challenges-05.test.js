...
/* ------------------------------------------------------------------------------------------------
CHALLENGE 1 - Review
Write a function that iterates over an` array of people ob`jects
...
const toLastNames = people => {
  let peoples = people.reduce((accumulator, person, index) => {
    accumulator[index] = person.firstName + ' ' + person.lastName;
    return accumulator;
  }, []);
  return peoples;
};
...
const returnNames = (arr) => {
  let newArr = [];
  newArr = arr.reduce((accumulator, value) => {
