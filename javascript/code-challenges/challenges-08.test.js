  ...
  const isNum = (input) => {
    if(typeof(input) === 'number') {
      ...
      let array = input.split('').map(Element => {
  ...
  const containsWorld = (input) => {
    return input.includes('world') ? true : false;
  };
  /* ------------------------------------------------------------------------------------------------
  CHALLENGE 5
  ...
  const isCapitalized = (str) => {
      ...
      let word = element.replace(/[^\w\s]/gi, '');
      
      if (word !== element.toLowerCase()) {
